...
public class AnalyticsRevenueAds
{
    ...
    private static void SendEventThreshold(ImpressionData data, AdFormat type)
    {
        ...
        bool isMaxDay = CheckConditionDay(time, RemoteConfigManager.Instance.config_max_day_send_revenue);

        if (rev >= RemoteConfigManager.Instance.min_value_revenue || isMaxDay)
        {
            // send event
            Firebase.Analytics.Parameter[] AdParameters =
            {
...
#endif
        form.AddField("packagename", GameConst.PACKAGE_NAME);
